...
Methods:
  ...
  def make_grid(self,rows,cols,**ka):
    """Incarnates *self* as grid and returns :attr:`grid`."""
  ...
  def make_axes(self,**ka):
    """Incarnates *self* as axes and returns :attr:`axes`."""
    assert self.axes is None and self.grid is None, 'Cell must be cleared before shaped'
...
Methods:
  ...
  def togglepause(self):
    """Toggles the pause status of the timer."""
  ...
  def onestep(self):
    """Performs a single shot of the timer if paused."""
    assert self.paused
    self.timer.start()

  def launch(self,anim):
    """Launches *self* and attaches *anim* to it. *anim* must be a :class:`matplotlib.animation.TimedAnimation` instance."""
  ...
  def shutdown(self):
    """Shutdowns *self* and stops and detachs its animation, if present."""
